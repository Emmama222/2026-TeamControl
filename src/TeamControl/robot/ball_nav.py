...
class FieldGeometryCache:
    """Detects a change in live field geometry (size mismatch awareness).
    ...
    and call `.refresh()` once per tick. The plain geometry helpers above
    (`is_target_in_field_box`, `sanitize_field_target`, ...) already read
    `get_live_bounds()` fresh every call, so they're always correct: this
    class is for callers that want to *react* to a change (e.g. recompute
    a derived zone) rather than just read the current value.
    """
    ...
    def refresh(self) -> bool:
        ...
        bounds = get_live_bounds()
        defence = get_live_defence()
...
def is_target_in_field_box(target, margin=ROBOT_RADIUS):
    ...
    Reads live field bounds fresh each call -- see FieldGeometryCache below
    for callers that want to detect a *change* in bounds instead.
    """
    if target is None:
        return False
...
def clamp_for_role(target_xy, is_goalie, margin=ROBOT_RADIUS,
    ...
    Args:
        ...
        own_goal_x: the exact x of this robot's own goal line, if the
            ...
            `sign * FIELD_LENGTH / 2`). Some callers compute field size
            from a different constant than this module's live field
            bounds -- passing this avoids that mismatch entirely. Falls
            back to the live field edge (`get_live_bounds()`) if omitted.
    ...
    if is_goalie:
        # Goalie must stay inside its own box -- clamp x to the live
        # max-advance distance from its own end line, same rule
        # goalie.py already uses.
        x_min, x_max, _, _ = get_live_bounds()
        max_advance = get_live_max_advance()
    ...
    for positive_side in (True, False):
        if is_in_penalty_box((tx, ty), positive_side=positive_side, margin=margin):
            defence_x, _, _, _ = get_live_defence()
            x_min, x_max, _, _ = get_live_bounds()
            if positive_side:
...
def compute_out_of_bounds_clearance(exit_xy, clearance_mm=500.0):
    ...
    get_live_bounds(), then pushes exit_xy `clearance_mm` back into the
...
def compute_ball_clearance_target(robot_xy, ball_xy, clearance_mm=500.0):
    ...
    Used to give a possessing opponent room instead of immediately pushing
...
def apply_boundary_braking(current_pos, vx, vy, max_speed=C.MAX_SPEED):
    ...
    goal_post_half_width = get_live_goal_half_width() + ROBOT_RADIUS
