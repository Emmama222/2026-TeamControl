...
from TeamControl.world import field_config


@pytest.fixture
def restore_live_geometry():
    """update_live_bounds/update_live_defence mutate module-global state --
    snapshot and restore it so these tests don't leak into others."""
    bounds_before = field_config.get_live_bounds()
    defence_before = field_config.get_live_defence()
    yield
    field_config._live_bounds = bounds_before
    field_config._live_defence = defence_before
...
class TestFieldGeometryCache:
    ...
    def test_detects_a_bounds_change(self, restore_live_geometry):
        cache = ball_nav.FieldGeometryCache()
        cache.refresh()
        field_config.update_live_bounds(12000.0, 9000.0)  # different from default 9000x6000
        assert cache.refresh() is True
        assert cache.refresh() is False
...
class TestClampForRole:
    def test_goalie_clamped_to_max_advance_positive_side(self):
        goal_x = 2250.0
        max_advance = field_config.get_live_max_advance()
    ...
    def test_non_goalie_pushed_out_of_own_penalty_box(self, restore_live_geometry):
        x_min, x_max, _, _ = field_config.get_live_bounds()
        defence_x, _, _, _ = field_config.get_live_defence()
...
class TestApplyBoundaryBrakingGoalPostZone:
    def test_zeroes_outward_velocity_in_goal_mouth_zone(self, restore_live_geometry):
        x_min, x_max, _, _ = field_config.get_live_bounds()
    ...
    def test_unaffected_outside_goal_mouth_width(self, restore_live_geometry):
        x_min, x_max, _, _ = field_config.get_live_bounds()
        ...
        goal_hw = field_config.get_live_goal_half_width()
