...
already age-compensated and radius-inflated by existing world-map logic.
...
The Qt map-debug tab can now include a `Voronoi map` layer. It is generated only
while the map-debug stream is enabled, and the engine already throttles map
render data to about `10 Hz`.
...
```text
[map] Voronoi overlay generated in 2.31 ms
```
